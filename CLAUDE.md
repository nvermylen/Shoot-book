...
Last ticket:         LENS-024 series COMPLETE through #55. Acceptance burst 2026-08-02: #47 runbook corrections, #48 embed fix + rls embed suite, #49 intake self-mail skip + throw isolation, #50 intake fails closed sans gateway (LENS-D-031), #51 agent_unavailable surfaced, #52 sync write-failures surfaced, #53 Booked-30d KPI real, #54 hydration/photographer-tz dates, #55 acceptance record. LENS-022 gates ALL GREEN (evidence in runbook Part D). Full 13-route sweep clean; backend endpoints healthy.
Next ticket:         None kicked off — LENS-024 runbook Parts A+B SIGNED OFF 2026-08-02. Prod schema completed (001–007; 003/004 were also missing, applied same day). Google connected 3-scope on acceptance account; 6 calendar events → 6 bookings; invoice book I1–I5 entered via UI (Playwright-driven); four-way reconciliation EXACT ($1,650/$1,050); one-refresh flip PASS ($1,250 after $400 payment); chase E2E PASS (3 real sends, correct parent routing, balance-not-face amounts, double-run idempotent via already_sent_today, per-invoice pause honored; send window is 8–10am local — tz-shim used for testing, restored). #48 fixed booking→invoice embed ambiguity (payments page hard-failed on full schema; embeds now smoke-tested in tests/rls). Vercel prod env gained GOOGLE_OAUTH_CLIENT_ID/SECRET + TOKEN_ENCRYPTION_KEY (were never set — connect 500'd). REMAINING: Part C intake (needs ANTHROPIC_API_KEY in Vercel prod — still missing — then test inquiries), B.6 disconnect-degradation (optional), then Morgan cutover. Known non-blocking bug to ticket: calendar sync per-event failures are silent (only unmatched surfaces).
Last migration:      migration_007_lead_thread_id.sql — applied to BOTH projects (prod 2026-08-02 after the wrong-project mixup, test earlier). 005/006/007 now verified present on prod via REST probe 2026-08-02.
Next migration:      migration_008 (none planned — 023c is UI-only)
Last PR branch:      LENS-024i-acceptance-record (merged as #55)
