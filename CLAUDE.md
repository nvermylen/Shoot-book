...
Next ticket:         None kicked off — owner executes the LENS-024 runbook (as amended #47). BLOCKING OWNER ACTION: apply migrations 005 → 006 → 007 IN ORDER to PROD (vvcuennzifsovbbylolx) — verified 2026-08-02 that prod has NONE of them (no invoice/payment tables; money surface has been schema-less since 022 shipped). Test project (eqqfukokwtwqwcqqeneo) already has all three. Then: three-box Google connect on acceptance account (nathan.vermylen@gmail.com, fresh "Nate" photographer; 6 acceptance clients seeded 2026-08-02, parent contacts on C2/C4 pending drawer edit), then Parts A–D sign-off. After sign-off: Morgan cutover, then next spec.
Last migration:      migration_007_lead_thread_id.sql (in repo via #43). PROD HAS NONE OF 005/006/007 — earlier "applied" claims were wrong (verified against both projects 2026-08-02, see LENS-024 runbook Step 0.2). Test project has all through 007.
