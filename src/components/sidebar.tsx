...
const groups = [
  {
    ...
    items: [
      ...
      { key: "/inquiries", label: "Inquiries", icon: Inbox, count: 3 },
    ],
  },
  {
    role: "Clients",
    desc: "Coordinator",
    items: [
      { key: "/clients", label: "All Clients", icon: Users, count: 42 },
  ...
  },
  {
    ...
    items: [
      { key: "/shoot-day", label: "Shoot Day", icon: Camera, count: 2 },
  ...
  },
  {
    ...
    desc: "Billing & Books",
    items: [
      { key: "/payments", label: "Payments", icon: DollarSign, count: 3 },
...
export function Sidebar({ metaphor = "subtle" }: { metaphor?: "subtle" | "medium" | "literal" }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "20px 20px 14px", borderBottom: "1px solid var(--rule)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            ...
            <div className="meta" style={{ fontSize: 10, marginTop: 2 }}>{DATA.photographer.studio}</div>
          </div>
        </div>
        {metaphor === "literal" && (
          <div
            ...
            {g.items.map((it) => {
              ...
              return (
                ...
                >
                  ...
                  <span>{it.label}</span>
                  {it.count ? <span className="nav-count num">{it.count}</span> : null}
                </div>
              );
            })}
          </div>
      ...
      <div style={{ padding: "12px 16px", borderTop: "1px solid var(--rule)", display: "flex", alignItems: "center", gap: 10 }}>
        <Avatar name={DATA.photographer.name} size={30} color="oklch(0.48 0.08 50)" />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 12.5, fontWeight: 500, color: "var(--ink)" }}>{DATA.photographer.name}</div>
          <div className="meta" style={{ fontSize: 10 }}>Owner · {DATA.photographer.tz}</div>
        </div>
