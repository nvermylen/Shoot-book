...
| `docs/architecture/ANTI_PATTERNS.md` | Never-do-this list with explanations | ✅ |
| `docs/architecture/DECISIONS_LOG.md` | Why key decisions were made | ✅ |
| `docs/architecture/DOMAIN_GLOSSARY.md` | Precise photography + agent terminology | ✅ |
| `docs/architecture/SECURITY.md` | Threat model, data classification, OAuth handling | ✅ |
| `docs/architecture/TESTING_STRATEGY.md` | Test pyramid, conventions | Phase 1 |
| `docs/architecture/DESIGN_SYSTEM.md` | Tokens, components, Tailwind patterns | Phase 1 |
| `PHASE_TEMPLATE.md` | Copy → `Phase1_Implementation_Plan.md` | Phase 1 |
| `FEATURE_SPEC_TEMPLATE.md` | Copy → per-feature specs | Phase 1 |
| `docs/personas/persona-end-user.md` (Morgan) | Real end user, design partner #1 | ✅ |
...
| File | Purpose | Day 1? |
|------|---------|--------|
| `docs/personas/PERSONA_PM.md` | Product thinking, scope rules, prioritization discipline | ✅ |
| `docs/personas/PERSONA_ARCH.md` | Schema, API contracts, system design standard | ✅ |
| `docs/personas/PERSONA_DEV.md` | Implementation patterns, file structure, TS standards | ✅ |
| `docs/personas/PERSONA_QA.md` | Test strategy, AC writing, edge case discipline | Phase 1 |
| `docs/personas/PERSONA_UX.md` | Interaction patterns, states, accessibility | Phase 1 |
...
### ⭐ `docs/architecture/AGENT_ARCHITECTURE.md` — KEYSTONE
...
- Specific entities → `docs/architecture/ERP_DATA_MODEL.md`
- Specific external systems → `docs/architecture/INTEGRATION_REGISTRY.md`
- Specific prompt versions → `AGENT_PROMPT_REGISTRY.md`

### `docs/architecture/ERP_DATA_MODEL.md`
...
**Boundary:** Entity definitions and relationships only. RLS → `docs/architecture/SECURITY.md`. Column types → migration SQL.
...
**Boundary:** External system contracts only. Internal data shape → `docs/architecture/ERP_DATA_MODEL.md`.
...
## Agent-on-ERP Additions to `docs/architecture/ANTI_PATTERNS.md`
...
- ❌ **Adding a new agent tool without updating `docs/architecture/INTEGRATION_REGISTRY.md`**
...
1. **`docs/architecture/AGENT_ARCHITECTURE.md`** — keystone. Everything else references it.
2. **`docs/architecture/ERP_DATA_MODEL.md`** — drives schema, migrations, and agent boundaries.
3. **`docs/personas/PERSONA_ARCH.md`** — refines #1 + #2 into the technical standard CC enforces.
4. **`CLAUDE.md`** — fill with project specifics (stack, env vars, agent-on-ERP framing, integration list, current build state).
5. **`docs/architecture/DOMAIN_GLOSSARY.md`** — lock photography terms + agent/ERP terms before any naming inconsistencies sneak in.
6. **`docs/architecture/ANTI_PATTERNS.md`** — fill with agent-on-ERP additions above.
7. **`docs/architecture/DECISIONS_LOG.md`** — log the foundational decisions you've already made (Replace vs Integrate, ERP source-of-truth, monorepo gateway pattern reuse, etc.). Don't lose these.
8. **`docs/personas/PERSONA_PM.md`, `docs/personas/PERSONA_DEV.md`** — fill with your taste; Dan's template is the skeleton.
9. **`docs/architecture/INTEGRATION_REGISTRY.md`** — start with just Gmail (first integration target). QuickBooks, Stripe, Calendar added when needed.
10. **`docs/architecture/SECURITY.md`** — fill, with extra attention to OAuth token storage and integration scopes.
...
- `docs/personas/PERSONA_QA.md`, `docs/personas/PERSONA_UX.md` filled
- `docs/architecture/DESIGN_SYSTEM.md` filled
- `docs/architecture/TESTING_STRATEGY.md` filled
